...
public class UserProfileScreen extends Application {
    public static List<Stage> profileStage = new java.util.ArrayList<>();
    ...
    public void start(Stage primaryStage) {
        primaryStage.setTitle("Expense Manager - Profile");
    ...
    private void handleSaveAction() {
        ...
        if (changingPassword) {
            ...
            if (!BCrypt.checkpw(currentPasswordInput, currentPassword)) {
            ...
            if (!newPassword.isEmpty()) {
                if (newPassword.length() > 8) {
                    showErrorMessage("Password must be 8 characters or less");
                    return;
                }
         ...
         * }
         */
        updateUserData(Session.getCurrentUser(), newUsername, newEmail, newPassword);
        setData();
        // Show success
    ...
    private void setData() {
        User currentUser = UserDAO.getUserById(Session.getCurrentUser());
        if (currentUser != null) {
    ...
    private void updateUserData(int id, String username, String email, String password) {
        ...
        User updatedUserDate = new User(id, username, email, hashPassword, null);
        boolean success = UserDAO.updateUser(updatedUserDate);
        if (success) {
            messageLabel.getStyleClass().add("success-message");
            messageLabel.setText("Profile updated successfully!");
